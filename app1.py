...
def toast_ok(msg: str, seconds: int = 5) -> None:
    ...
    Aqui usamos um placeholder (success) que fica na tela e some depois.
    """
    ph = st.empty()
    ph.success(f"OK ✅ {msg}")
...
CATEGORIAS = [
    ...
    "Desnecessário", "Beleza", "Funcionários", "Trabalho", "Vestuário", "Acessórios", "Outros"
]
...
def atualizar_varios(df_edit: pd.DataFrame) -> Tuple[int, List[str]]:
    """
    Atualiza vários itens (planilha).
    Espera colunas:
        ...
        with conn.cursor() as cur:
            for idx, row in df_edit.iterrows():
                ...
                try:
                    ...
                    erros.append(f"Linha {idx+1}: Data inválida (use AAAA-MM-DD).")
                    continue

                v = parse_brl(row.get("Valor", ""))
                ...
                if dp_raw:
                    try:
                        ...
                        erros.append(f"Linha {idx+1}: Data_Pagamento inválida (use AAAA-MM-DD).")
                        continue
...
with tab_lancar:
    ...
    # Troca o number_input por text_input (evita 0,001500,00 quando digita em cima do 0,00)
    def valor_input(label: str, key: str, disabled: bool) -> str:
        default = st.session_state.get(key, "0,00")
    ...
    if isinstance(df_previa, pd.DataFrame) and not df_previa.empty:
        # data_editor: mostra Valor como texto BR, e parseia de volta no salvar.
        ...
        if st.button("Salvar parcelas 💾", type="primary", use_container_width=True, key="l_salvar"):
            ...
            for idx, row in edited.iterrows():
                ...
                try:
                    ...
                    erros.append(f"Linha {idx+1}: Data inválida '{row.get('Data','')}' (use dd/mm/aaaa).")
                    continue
                ...
                if dp_txt:
                    try:
                        ...
                        erros.append(f"Linha {idx+1}: Data_Pagamento inválida '{row.get('Data_Pagamento','')}' (use dd/mm/aaaa).")
                        continue
...
with tab_editar_lote:
    ...
    if df.empty:
        ...
        # Normaliza tipos para o data_editor (evita erro de compatibilidade)
        df_edit["ID"] = df_edit["ID"].astype(str)
        df_edit["DataISO"] = pd.to_datetime(df_edit["DataISO"], errors="coerce").dt.strftime("%Y-%m-%d").fillna("")
        df_edit["Data_PagamentoISO"] = pd.to_datetime(df_edit["Data_PagamentoISO"], errors="coerce").dt.strftime("%Y-%m-%d").fillna("")
        for c in ["Descricao","Prestacao","Forma_de_Pagamento","Categoria","Conta_Corrente","Status"]:
            df_edit[c] = df_edit[c].astype(str).fillna("")
        ...
        with c2:
            if st.button("Salvar alterações em lote 💾", type="primary", use_container_width=True, key="el_save"):
                ...
                if erros:
                    st.error(
                        "Alguns itens não salvaram:\n\n- "
                        + "\n- ".join(erros[:20])
                        + ("" if len(erros) <= 20 else f"\n\n(+{len(erros)-20} erros)")
                    )
...
with tab_excluir:
    ...
    if not rows:
        ...
        id_options = [int(r[0]) for r in rows]

        def _fmt(i: int) -> str:
            for r in rows:
                if int(r[0]) == int(i):
                    _, desc, val, dt, prest, status = r
                    data_br = pd.to_datetime(dt).strftime("%d/%m/%Y")
                    prest_txt = f" [{prest}]" if prest else ""
                    return f"{data_br} • R$ {br_money(val)} • {desc}{prest_txt} • {status or 'Pendente'}"
            return str(i)

        id_del = st.selectbox("Selecione", options=id_options, key="d_id", format_func=_fmt, index=0)
